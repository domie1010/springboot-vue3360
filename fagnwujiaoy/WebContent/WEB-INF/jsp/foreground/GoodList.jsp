...
<table width="100%" height="27" border="0" cellpadding="0" cellspacing="0" >
  <tr>
    ...
    <td class="shopcarttitle">您有以下房屋 &nbsp;&nbsp;<a href="sys/request/goodAdd">添加</a></td>
  </tr>
</table>
<br>
<br>
<table width="100%" border="0" cellpadding="3" cellspacing="0" class="shopcart">
		  ...
		  <c:forEach items="${pager.datas }" var="good" varStatus="vs">
                <tr >
	                ...
	                <td>${good['goodType'] }</td>
	                <td>${good['goodIntroduce'] }</td>
	                <c:if test="${good['status'] eq 1 }">
	                	<td>待售</td>
	                </c:if>
	                <c:if test="${good['status'] eq 2 }">
	                	<td>交易中</td>
	                </c:if>
	                 <c:if test="${good['status'] eq 3 }">
	                	<td>已售</td>
	                </c:if>
	                <td>
	                 <c:if test="${good['status'] eq 2 }">
	                 	<a href="sys/good/success?id=${good['id'] }">同意</a>
	                 	<a href="sys/good/failed?id=${good['id'] }">拒绝</a>
	                 </c:if>
	                	<a href="sys/good/queryOneLord?id=${good['id'] }">查看</a>
	                 </td>
                </tr>
 			 </c:forEach>
</table>
...
<script type="text/javascript">
	...
	function deleteC(id){
		if(window.confirm("您确定要退货吗?")){
			window.location.href="sys/bargain/update?id="+id+"&status=3";
			return true;
		}
		return false;
	}

...
<div id="channelmenumain">
	...
	<a href="sys/good/query" target="_self" class="channelmenu_main" id="channelmenu_main_1" onMouseOver="showSubMenu('channelmenu_main','channelmenu_sub','1')">房屋信息</a>

	<a href="sys/bargain/query?customName=${sessionScope['User']['account'] }" target="_self" class="channelmenu_main" id="channelmenu_main_2" onMouseOver="showSubMenu('channelmenu_main','channelmenu_sub','2')">订单管理</a>
	
	<a href="sys/message/query" target="_self" class="channelmenu_main" id="channelmenu_main_4" onMouseOver="showSubMenu('channelmenu_main','channelmenu_sub','4')">广告信息</a>
	
	<a href="sys/request/background/index" target="_self" class="channelmenu_main" id="channelmenu_main_5" onMouseOver="showSubMenu('channelmenu_main','channelmenu_sub','5')">后台系统</a>
	<c:if test="${sessionScope['User']['type'] eq 3 }">
		<a href="sys/query" target="_self" class="channelmenu_main" id="channelmenu_main_6" onMouseOver="showSubMenu('channelmenu_main','channelmenu_sub','6')">用户信息</a>
	</c:if>
	<c:if test="${sessionScope['User']['account'] ne null }">
		<a href="sys/good/queryLord?goodLord=${sessionScope['User']['account'] }" target="_self" class="channelmenu_main" id="channelmenu_main_3" onMouseOver="showSubMenu('channelmenu_main','channelmenu_sub','3')">我的房屋</a>
	</c:if>
...
<form id="searchform" method="post" action="sys/good/query">
  ...
  房屋搜索：<select name="searchType">
      <option value="0" >选择搜索类型</option>
	  <option value="房屋名称">房屋名称</option>
	  <option value="房屋价格">房屋价格</option>
	  <option value="房屋大小">房屋大小</option>
    </select>  
  </div>
  <div class="searchform">
  </div>
...
<div style="position:relative;height:25px;border-bottom:1px #d4d4d4 solid;margin:-1px -1px 0px -1px;background:url(image/image/foreground/top.gif) left no-repeat">
	...
	<a href="index-6.html" style="font:12px/25px simsun;color:#555;margin-right:8px;display:inline">更多&gt;&gt;</a>
	</div>
</div>
...
${messageGood }
<c:forEach items="${pagerGood.datas }" var="good" varStatus="vs">
<div class="shoplist_10">
		<div class="fang" style="width:100px;height:100px">
			<div class="picFit" style="width:100px;height:100px">
			<a href="sys/good/queryOne?id=${good['id'] }" target="_self" ><img src="upload/upload/${good['url'] }" style="width:100px;height:100px" border="0" /></a>
			</div>
		</div>
		<div class="prop">
		<span class="price">￥${good['goodPrice'] }</span><br />
		</div>
...
<form action="sys/login" method="post" onsubmit="return check();" id="login">
...
<li id="li" style="display: none">验证:<input type="password" name="safe" id="safe" style="width: 120px; margin-top: 5px;"></li>

<li>身份:
	<select name="type" onchange="trWay(this.value);">
...
<div style="position:relative;height:30px;border:0px;margin:-1px -1px 0px -1px;background:url(image/image/foreground/top-2.gif) left no-repeat">
	...
	<a href="sys/message/query" style="font:12px/28px simsun;color:#555;margin-right:8px;display:inline">更多&gt;&gt;</a>
	</div>
</div>
<div style="padding:10px">
...
<ul class="newslist">
${messageMessage }
<c:forEach items="${pagerMessage.datas }" var="message" varStatus="vs" begin="0" end="5">
<li class="newslist"><a href="javascript:;" target="_self" class="newslist"   >${message['userMessage'] }</a></li>
</c:forEach>
...
<div id='pdv_3735' class='pdv_class'  title='热卖房屋' style='width:195px;height:274px;top:193px;left:705px; z-index:10'>
...
<div style="position:relative;height:30px;border:0px;margin:-1px -1px 0px -1px;background:url(image/image/foreground/top-2.gif) left no-repeat">
	...
	<div style="float:right;width:50px;height:30px;text-align:right;background:url(image/image/foreground/top-2.gif) -750px no-repeat">
	<a href="sys/good/query" style="font:12px/28px simsun;color:#555;margin-right:8px;display:inline">更多&gt;&gt;</a>
	</div>
</div>
<div style="padding:10px">


<ul class="shophotlist">

<c:forEach items="${pagerGood.datas }" var="good" varStatus="vs">
<li class="shophotlist" style="background: transparent url(image/image/foreground/no1.gif) left no-repeat;padding-left:23px">

<a href="javascript:;" target="_self" class="shophotlist"   >${good['goodName'] } </a>
</li>
</c:forEach>
